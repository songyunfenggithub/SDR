...
*******************************************************************************/
void ICACHE_FLASH_ATTR
user_tcp_send_data(char* pbuf, u16 len)
{
	oldt = system_get_time();
	espconn_send(pespconn, pbuf, len);
}
...
user_check_ip(void)
{
    ...
    if (wifi_station_get_connect_status() == STATION_GOT_IP && ipconfig.ip.addr != 0)
   {
      ...
      user_tcp_conn.state = ESPCONN_NONE;
       ...
       user_tcp_conn.proto.tcp->remote_port =3456;  // remote port
