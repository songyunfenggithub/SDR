...
cuda_CFilter3 clscudaMainFilter3;

__global__ void
cuda_core_Filter3(
	const ADC_DATA_TYPE* src, int stage, unsigned int srclen,
	FILTTED_DATA_TYPE* decimation_cache1, const FILTER_CORE_DATA_TYPE* core1, unsigned int decimation_factor_bit1, unsigned int corelen1,
	FILTTED_DATA_TYPE* decimation_cache2, const FILTER_CORE_DATA_TYPE* core2, unsigned int decimation_factor_bit2, unsigned int corelen2,
	FILTER_CORE_DATA_TYPE* filtted_result1,
	FILTER_CORE_DATA_TYPE* filtted_result2,
	float scale
)
{
...
__global__ void
cuda_core_Filter3_Demodulator(FILTER_CORE_DATA_TYPE* filtted_result, FILTTED_DATA_TYPE* demodulator_result,
...
void cuda_CFilter3::Init(CFilter* f)
{
	...
	size_t filter_core_size1 = rootFilterInfo1->CoreLength * sizeof(FILTER_CORE_DATA_TYPE);
	...
	size_t filter_core_size2 = rootFilterInfo2->CoreLength * sizeof(FILTER_CORE_DATA_TYPE);
	...
	*/
	printf("Cuda Filter3 Init.\r\n");
}

void cuda_CFilter3::UnInit(void)
{
	if (this == NULL)return;
	cudaError_t err = cudaSuccess;
	...
	if (d_Filter_Core1 != NULL) {
		...
		if (err != cudaSuccess) {
			printf("cudaFree d_Filter_Core1 failed!\r\n");
			//goto Error;
		}
		d_Filter_Core1 = NULL;
	}
	if (d_Filter_Core2 != NULL) {
		...
		if (err != cudaSuccess) {
			printf("cudaFree d_Filter_Core2 failed!\r\n");
			//goto Error;
		}
		d_Filter_Core2 = NULL;
	}
	if (d_Filtted_Result1 != NULL) {
		...
		if (err != cudaSuccess) {
			printf("cudaFree d_Filtted_Result1 failed!\r\n");
			//goto Error;
		}
	...
	if (d_Decimation_Cache1 != NULL) {
		...
		if (err != cudaSuccess) {
			printf("cudaFree d_Decimation_Cache1 failed!\r\n");
			//goto Error;
		}
	...
	if (d_Decimation_Cache2 != NULL) {
		...
		if (err != cudaSuccess) {
			printf("cudaFree d_Decimation_Cache2 failed!\r\n");
			//goto Error;
		}
	...
	printf("cuda_CFilter3 UnInit.\r\n");
}

void cuda_CFilter3::Filtting(void)
{
	...
	if (err != cudaSuccess) {
		printf("3cudaMemcpy d_SrcData failed!\r\n");
	}
	...
	//printf("CUDA kernel launch with %d blocks of %d threads\n", blocksPerGrid, threadsPerBlock);
		...
		//FILTTED_DATA_TYPE* decimation_cache1, const FILTER_CORE_DATA_TYPE* core1, unsigned int decimation_factor_bit1, unsigned int corelen1,
		//FILTTED_DATA_TYPE* decimation_cache2, const FILTER_CORE_DATA_TYPE* core2, unsigned int decimation_factor_bit2, unsigned int corelen2,
		//FILTER_CORE_DATA_TYPE* filtted1_result,
		//FILTER_CORE_DATA_TYPE* filtted2_result

	cuda_core_Filter3 << <threadsPerBlock, blocksPerGrid >> > (
	...
	if (err != cudaSuccess) {
		printf("3cudaFilter launch failed: %s\r\n", cudaGetErrorString(err));
	}
	// any errors encountered during the launch.
	err = cudaDeviceSynchronize();
	if (err != cudaSuccess) {
		printf("3cudaDeviceSynchronize returned error code %d after launching cudaFilter!\r\n", err);
	}

	CData* target_Data = (CData*)TargetData2;
	...
	if (err != cudaSuccess) {
		printf("3cudaMemcpy h_Filtted_Result failed!\r\n");
	}
	UINT T;
